...
router.get('/documents', authenticate, async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, docs, 'DMS documents retrieved', 200, {
      total,
      page: parseInt(page),
...
router.get('/records', authenticate, async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, records, 'MIS records retrieved', 200, {
      total,
      page: parseInt(page),
      pages: Math.ceil(total / limit)
    });
