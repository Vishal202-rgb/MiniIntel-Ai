    ...
    <div className="p-6 max-w-7xl mx-auto text-gray-800 dark:text-neutral-200">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-neutral-400">Track and monitor all system and user activities.</p>
        </div>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white dark:bg-dark-card border border-gray-200 dark:border-neutral-800 p-5 rounded-xl">
          <h3 className="text-sm font-medium text-gray-600 dark:text-neutral-400 mb-1">Total Events</h3>
          <p className="text-3xl font-bold text-gray-900 dark:text-white">{loading ? '-' : stats.totalEvents}</p>
        </div>
        <div className="bg-white dark:bg-dark-card border border-gray-200 dark:border-neutral-800 p-5 rounded-xl">
          <h3 className="text-sm font-medium text-gray-600 dark:text-neutral-400 mb-1">Successful</h3>
          <p className="text-3xl font-bold text-green-400">{loading ? '-' : stats.successful}</p>
        </div>
        <div className="bg-white dark:bg-dark-card border border-gray-200 dark:border-neutral-800 p-5 rounded-xl">
          <h3 className="text-sm font-medium text-gray-600 dark:text-neutral-400 mb-1">Failed</h3>
          <p className="text-3xl font-bold text-red-400">{loading ? '-' : stats.failed}</p>
        </div>
        <div className="bg-white dark:bg-dark-card border border-gray-200 dark:border-neutral-800 p-5 rounded-xl">
          <h3 className="text-sm font-medium text-gray-600 dark:text-neutral-400 mb-1">Active Users</h3>
      ...
      <div className="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-neutral-800 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-12 flex flex-col items-center justify-center text-gray-500 dark:text-neutral-500">
          ...
          <div className="p-12 text-center text-gray-500 dark:text-neutral-500">
            No audit logs found.
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-gray-700 dark:text-neutral-300">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-neutral-800">
                {logs.map((log) => (
                  <tr key={log._id} className="hover:bg-gray-100 dark:hover:bg-[#222] transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-neutral-400">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 bg-neutral-800 text-gray-800 dark:text-neutral-200 rounded-md text-xs font-mono">
                        {log.action}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-gray-600 dark:text-neutral-400">
                      {log.resource || '-'}
                    </td>
                    <td className="px-6 py-4">
                      {log.status === 'SUCCESS' ? (
                    ...
                    <td className="px-6 py-4 max-w-xs truncate text-gray-500 dark:text-neutral-500" title={JSON.stringify(log.details)}>
