...
const AdminDashboard = () => {
  ...
  return (
    <div className="p-5 max-w-[1400px] mx-auto space-y-8">
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-4 rounded-lg flex items-center gap-3 border border-red-100 dark:border-red-900/30">
      ...
      {successMessage && (
        <div className="bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 p-4 rounded-lg flex items-center gap-3 border border-green-100 dark:border-green-900/30">
      ...
      <div className="bg-white dark:bg-dark-card rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60">
              {filteredUsers.map(u => (
                <tr key={u._id} className="hover:bg-blue-50/50 dark:hover:bg-dark-card/30 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-slate-400 dark:text-slate-400 whitespace-nowrap">{formatDate(u.createdAt)}</td>
        ...
        {filteredUsers.length === 0 && (
          <div className="py-16 text-center">
            ...
            <p className="text-slate-400 dark:text-slate-400 font-medium text-base">
              {userSearch || roleFilter !== 'all' ? 'No users match your filters.' : 'No users registered yet.'}
            </p>
            <p className="text-slate-400 dark:text-slate-400 text-sm mt-1">Users will appear here automatically when they register.</p>
          </div>
        )}
      </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-dark-card rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800/60">
                {documents.slice(0, 8).map(doc => (
                  <tr key={doc._id} className="hover:bg-purple-50/50 dark:hover:bg-dark-card/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-400 dark:text-slate-400 whitespace-nowrap">{formatDate(doc.uploadedAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {documents.length === 0 && (
            <div className="py-16 text-center">
              ...
              <p className="text-slate-400 dark:text-slate-400 font-medium text-base">No documents uploaded yet.</p>
            </div>
          )}
        </div>
            ...
            <div className="p-5 space-y-5">
              {systemHealth ? Object.entries(systemHealth).map(([service, status]) => (
                <div key={service} className="flex justify-between items-center">
                  <span className="text-sm font-medium text-neutral-600 dark:text-slate-400 capitalize">{service}</span>
                  <HealthDot status={status} />
                </div>
              )) : (
                <p className="text-sm text-slate-400 text-center py-2">Health data unavailable.</p>
              )}
            </div>
          </div>
              ...
              {auditLogs.slice(0, 6).map((log, i) => (
                <div key={log._id || i} className="flex gap-3 items-start">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-400 mt-0.5">{formatDate(log.timestamp || log.createdAt)}</p>
                  </div>
                </div>
              ))}
              {auditLogs.length === 0 && (
                <div className="text-center py-10">
                  ...
                  <p className="text-sm font-medium text-slate-400 dark:text-slate-400">No recent activity recorded.</p>
                </div>
              )}
            </div>
            {auditLogs.length > 0 && (
